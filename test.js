 ...
 * v==V
 */
const overlappingIntervals = (u, U) => (v, V) => u <= V && v <= U;

/**
 * for a given:
 * @param ref reference rectangle
 * @returns a function which, for a given dimension ('x' or 'y'), returns true if the given rectangle r overlaps ref in that dimension.
 * /Hint/: use dim.toUpperCase() to get the max extent in dim
 */
function overlap(ref) {
  return (dim) => (r) =>
    overlappingIntervals(ref[dim], ref[dim.toUpperCase()])(
      r[dim],
      r[dim.toUpperCase()]
    );
}
...
function rectanglesOverlappingSelection(selection, rects) {
  return rects.filter(
    (r) => overlap(selection)("x")(r) && overlap(selection)("y")(r)
  );
}
...
function sumAreasOverlappingSelection(selection, rects) {
  return rects
    .filter((r) => overlap(selection)("x")(r) && overlap(selection)("y")(r))
...
function computeBoundsRectanglesOverlappingSelection(selection, rects) {
  const overlappingRects = rects.filter(
    (r) => overlap(selection)("x")(r) && overlap(selection)("y")(r)
  );
  if (overlappingRects.length === 0) {
    return selection;
  }
  const bounds = overlappingRects.reduce(
    (bounds, r) => boundingRect(bounds, r),
    overlappingRects[0]
  );
  return {
    x: bounds.x,
